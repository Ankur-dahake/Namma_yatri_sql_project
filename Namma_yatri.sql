...
-- all trips enculde successful and canclled trips  
select COUNT(distinct tripid)as total_trip from trips_details4;

-- cheaked weither we have duplicate trip id 
...
) as searches from trips_details4;


--Q7.total searches for quotes

select SUM(searches_for_quotes) as searches from trips_details4;
...
select COUNT(*) As Complete_ride from trips_details4
where end_ride=1;
...
select AVG(distance) as Avg_distane from trips;
...
select top 1 COUNT(distinct t.tripid) as cnt ,p.method from trips as t
...
select top 1 t.fare,p.method  from trips as t
join payment as p
...
select *, dense_rank() over (order by earning desc ) rnk from(
 select SUM(fare) as earning  ,driverid from trips 
 group by driverid
 ) as a )as b
 where rnk <=5;
...
select round(SUM(searches_got_estimate)*100/SUM(searches),2) as estimate_rate_pct from trips_details4;


-- estimate to search for quote rates
select round(SUM(searches_for_quotes)*100/SUM(searches_got_estimate),2) as quote_rate_pct from trips_details4;

select * from trips_details4

-- quote acceptance rate

select round(SUM(searches_got_quotes)*100/SUM(searches_for_quotes),2) as quote_rate_pct from trips_details4;
...
-- conversion rate
select round(SUM(end_ride)*100/SUM(searches),2) as estimate_rate_pct from trips_details4;

-- which location got the higest number of trip in each duration present 
select * from (
...
select * from(
select * ,dense_rank() over(order by fare desc) as rnk from
...
-- from which location driver cancel most od ride .
select * from ( select * ,RANK() over(order by cancel_by_driver desc ) as rnk from(
select loc_from ,COUNT(distinct tripid) - SUM(customer_not_cancelled) as cancel_by_driver from trips_details4
group by loc_from ) as a )as b
where rnk=1;

-- which duration got the highest trips and fares

select * from ( select * , dense_rank() over ( order by Number_of_trip desc) as rnk from 
(select duration , COUNT(distinct tripid) as Number_of_trip  from trips
group by duration) as a) as b 


select * from ( select * , dense_rank() over ( order by fare desc) as rnk from 
(select duration , sum(fare) as fare  from trips
group by duration) as a ) as b 


select * ,dense_rank() over(order by total_fare desc,cnt desc ) rnk from (
		select duration , COUNT(tripid) as cnt  , SUM(fare) as total_fare  from trips 
